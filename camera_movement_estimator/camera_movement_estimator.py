...
class CameraMovementEstimator:
    ...
    def get_camera_movement(self, frames, read_from_stub=False, stub_path=None):
        ...
        if read_from_stub and stub_path is not None and os.path.exists(stub_path):
            print("acquiring camera movement from stub")
    ...
    def draw_camera_movement(self, frame, camera_movement_per_frame):
        ...
        for frame_num, frame in enumerate(frame):
            ...
            cv2.putText(frame,f"X Camera Movement: {x_movement:.2f}",(10, 30),cv2.FONT_HERSHEY_SIMPLEX,1,(0,0,0),thickness=3)
            cv2.putText(frame,f"Y Camera Movement: {y_movement:.2f}",(10, 60),cv2.FONT_HERSHEY_SIMPLEX,1,(0,0,0),thickness=3)
